---
id: blog-1787131501460-the-5-second-purge-how-cloudfl
...
>
> Meta operates some of the largest cache deployments in the world, including TAO and Memcache, serving trillions of cache operations daily across hundreds of data centers. Cache inconsistency caused real production incidents, including a messaging split-brain where messages to the same user were sent to different storage regions, losing message completeness.
>
> | | |
> |---|---|
> | **Challenge** | Cache invalidation in dynamic caches is inherently racy — data gets mutated on both the read (cache fill) and write (cache invalidation) paths. A classic race condition: a cache fill arrives with stale data AFTER an invalidation has already written newer data, leaving the cache permanently inconsistent. At Meta's scale (1+ quadrillion queries/day on TAO), even a 99.9999% consistency rate meant millions of inconsistent cache entries. |
> | **Solution** | Meta built Polaris, a consistency observability service that monitors cache consistency at up to 10 nines of precision by acting as a client receiving invalidation events and checking replicas. They also built consistency tracing — a stateful logging library embedded in cache services that only traces mutations in the critical window where race conditions can occur (between an invalidation arriving and the next cache quiescence). This targeted approach avoids the impossible task of logging every cache state change while catching the bugs that matter. |
> | **Outcome** | Improved TAO cache consistency from 99.9999% (six nines) to 99.99999999% (ten nines) — meaning less than 1 inconsistent read per 10 billion operations after 5 minutes. A specific bug involving rare interleaving of database transactions and error handling code that caused permanent staleness was found and diagnosed in under 30 minutes using consistency tracing, compared to weeks of manual debugging. |
> | **Lesson** | You can't fix what you can't measure. Building precise, false-positive-free consistency observability (Polaris) was the prerequisite for all improvements. The key insight is that you don't need to log everything — just trace mutations in the narrow window where races between fills and invalidations can cause permanent inconsistency. This turns an intractable debugging problem into a targeted one. |
