...
async function main() {
  ...
  // Merge per-channel test files from data/tests/ into a single tests.json for the build
  {
    ...
    const filtered = filterTests(allTests, questions);
    fs.writeFileSync(path.join(OUTPUT_DIR, 'tests.json'), JSON.stringify(filtered, null, 0));
    console.log(`   ✓ tests.json (${filtered.length} tests merged from data/tests/)`);
  }
  ...
  for (const f of ['bot-activity.json', 'github-analytics.json', 'bot-monitor.json', 'similar-questions.json']) {
    const dest = path.join(OUTPUT_DIR, f);
    if (!fs.existsSync(dest)) {
      fs.writeFileSync(dest, JSON.stringify([], null, 0));
    }
  }

  // Merge per-post blog files from data/blog-posts/ into blog-posts.json for the build
  {
    const blogDir = path.join(DATA_DIR, 'blog-posts');
    let allPosts = [];
    if (fs.existsSync(blogDir)) {
      for (const f of fs.readdirSync(blogDir).filter(f => f.endsWith('.json'))) {
        try { allPosts.push(JSON.parse(fs.readFileSync(path.join(blogDir, f), 'utf8'))); } catch {}
      }
    }
    fs.writeFileSync(path.join(OUTPUT_DIR, 'blog-posts.json'), JSON.stringify(allPosts, null, 0));
    console.log(`   ✓ blog-posts.json (${allPosts.length} posts merged from data/blog-posts/)`);
  }
