...
const practiceSubNav: NavItem[] = [
  { id: 'voice',      label: 'Voice Interview', icon: Mic,    path: '/voice-interview', description: 'AI mock interviews',  shortcut: 'V' },
...
function getActiveSection(location: string): string {
  ...
  if (location === '/channels' || location.startsWith('/channel/') || location === '/certifications' || location.startsWith('/certification/') || location === '/my-path' || location === '/learning-paths') return 'learn';
  if (location.startsWith('/voice') || location.startsWith('/test') || location.startsWith('/coding') || location.startsWith('/code') || location === '/review' || location === '/flashcards') return 'practice';
  if (location === '/profile' || location === '/badges' || location === '/bookmarks' || location === '/about') return 'profile';
  return 'home';
}

// ─── Sub-nav item (extracted to avoid inline arrow in map) ─────────────────────

const SubNavItem = React.memo(function SubNavItem({ item, index, location, onSelect }: {
  item: NavItem;
  index: number;
  location: string;
...
// ─── Main nav item (extracted to avoid inline arrow in map) ────────────────────

const MainNavButton = React.memo(function MainNavButton({ item, activeSection, showMenu, onNavClick }: {
  item: NavItem;
  ...
  const Icon = item.icon;
  const handleClick = useCallback(() => onNavClick(item), [onNavClick, item]);
...
export function MobileBottomNav() {
  ...
  }, [showMenu, setLocation]);

  const closeMenu = useCallback(() => setShowMenu(null), []);
...
export function UnifiedMobileHeader({ title, showBack, onSearchClick }: UnifiedMobileHeaderProps) {
  ...
  const goProfile = useCallback(() => setLocation('/profile'), [setLocation]);
