/**
 * LinkedIn-style Mobile Bottom Navigation
...
interface LinkedInMobileNavProps {
  onCreateClick?: () => void;
}

export function LinkedInMobileNav({ onCreateClick }: LinkedInMobileNavProps) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 lg:hidden">
      {/* Clean white background with subtle shadow - LinkedIn style */}
      ...
      >
        {navItems.map((item) => {
          ...
          if (item.isCenter) {
            // Center action button - LinkedIn style "Post" button
            return (
              <button
                key={item.id}
          ...
          return (
            ...
            >
              {/* Active indicator - top line like LinkedIn */}
              {active && (
                <motion.div
                  layoutId="linkedin-nav-indicator"
