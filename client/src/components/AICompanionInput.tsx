      ...
      {hasMessages && (
       ...
       <div className="flex gap-2">
         <input
           ...
           maxLength={maxLength}
           aria-invalid={!!error}
           aria-describedby={error ? 'companion-error' : (inputMessage.length > maxLength * 0.8 ? 'companion-counter' : undefined)}
           className="flex-1 px-4 py-2 bg-background border border-border rounded-full text-sm focus:outline-none focus:border-primary disabled:opacity-[0.38]"
         />
         {!voiceMode && (
           <button
             onClick={onSend}
             disabled={!inputMessage.trim() || isGenerating}
             className="p-2 bg-gradient-to-r from-purple-500 to-pink-500 text-white rounded-full hover:opacity-90 transition-opacity disabled:opacity-[0.38] disabled:cursor-not-allowed"
           >
             {isGenerating ? (
       ...
       {error && (
         <p id="companion-error" role="alert" className="mt-1 text-xs text-red-500">
           {error}
         </p>
       )}
       {inputMessage.length > maxLength * 0.8 && (
         <p id="companion-counter" className={`mt-1 text-xs ${inputMessage.length >= maxLength ? 'text-red-500' : 'text-muted-foreground'}`}>
           {inputMessage.length}/{maxLength} characters
         </p>
       )}
       {voiceMode && (
