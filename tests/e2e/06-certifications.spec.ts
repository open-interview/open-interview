...
test.describe('Certifications — /certifications', () => {
  ...
  test('Start Practice navigates to valid page not 404 (P1-07)', async ({ page }) => {
    const cards = page.locator('[class*="rounded-xl"][class*="cursor-pointer"]');
    if (await cards.count() === 0) return;
  ...
  test('last cert card row visible above mobile nav (P2-02)', async ({ page }) => {
...
test.describe('Certification Practice — /certification/:id', () => {
  ...
  test('/certification/aws-solutions-architect loads or gracefully 404s (P1-07)', async ({ page }) => {
